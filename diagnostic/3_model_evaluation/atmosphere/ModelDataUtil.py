...
class ModelDataReader:
    ...
    def _ensure_datetime64(self, da: xr.DataArray, frequency: str) -> xr.DataArray:
        """
        Ensure 'time' coordinate is pandas-compatible datetime64[ns] and normalize
        if daily or coarser. Safe for cftime calendars.
        """
        # Fast path: already datetime64
        is_dt64 = isinstance(da.indexes.get("time"), pd.DatetimeIndex)
    
        if not is_dt64:
            # Try converting the calendar to proleptic_gregorian and return numpy datetimes
            try:
                # convert_calendar works on Dataset; round-trip through a tiny ds
                tmp = xr.Dataset({"_v": da})
                tmp2 = xr.convert_calendar(tmp, "proleptic_gregorian", use_cftime=False)
                da = tmp2["_v"]
            except Exception:
                # Fallback: best-effort manual conversion (may fail for extreme dates)
                # Keep ordering and uniqueness
                try:
                    # Build pandas Timestamps from cftime objects
                    dt64 = pd.to_datetime([str(t) for t in da["time"].values], errors="coerce")
                    if dt64.isna().any():
                        # As a last resort, keep cftime and raise a clear error
                        raise ValueError("Some times are outside numpy datetime64 range.")
                    da = da.assign_coords(time=("time", dt64.to_numpy()))
                except Exception as e:
                    # If you can operate on cftime directly, it’s better to not force datetime64.
                    raise RuntimeError(
                        "Failed to coerce 'time' to datetime64[ns] (numpy range issue). "
                        "Consider keeping cftime and using .dt accessors."
                    ) from e
    
        # ---- Normalize by frequency ----
        freq = (frequency or "").lower()
        if freq in {"monthly", "mon", "m", "1m", "1me", "yearly", "annual", "y"}:
            # Snap to month start (e.g., 2012-03-01T00:00)
            # If you already have a helper, keep using it; otherwise:
            t = pd.to_datetime(da["time"].values)
            month_start = (t.to_period("M").to_timestamp(how="S"))  # start of month
            da = da.assign_coords(time=("time", month_start.to_numpy()))
        elif freq in {"daily", "day", "d"}:
            # Normalize to midnight
            t = pd.to_datetime(da["time"].values)
            da = da.assign_coords(time=("time", t.normalize().to_numpy()))
